package com.dingli.diandiaan.newProject.utils;
...
public class CrashManager implements Thread.UncaughtExceptionHandler {
    ...
    public static CrashManager getInstance(Application application) {
        ...
        if (inst == null) {
            synchronized (CrashManager.class) {
                inst = instance;
                if (inst == null) {
                    inst = new CrashManager(application);
                    instance = inst;
                }
            }
        }
    ...
    private boolean handleException(final Throwable ex) {
        ...
        try{
            //收集设备参数信息
            collectDeviceInfo(mApplication.getApplicationContext());
            //输出带行数的具体错误信息
    ...
    private String saveCatchInfo2File(Throwable ex) {
        ...
        try {
            ...
            if (SDCardUtils.isSDCardEnable()) {
                String path = SDCardUtils.getSDCardPath()+"bestkeep/crash/";
